// TEMP debug route -- calls extractAuctionStats() directly against Cucumber's real
...
import { extractAuctionStats } from '../../player/sync/route'
...
export async function GET() {
  ...
  const { error: updateError } = await supabase
    .from('player_data')
    .update({ auction_stats: auctionStats, updated_at: new Date().toISOString() })
    .eq('profile_id', CUCUMBER_PROFILE_ID)

  return NextResponse.json({
    extracted: auctionStats,
