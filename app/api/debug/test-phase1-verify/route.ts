...
export async function GET() {
  ...
  const { data: agc } = await supabase.from('activity_gear_categories').select('activity_key, item_category')
  const { data: pt } = await supabase.from('progression_tiers').select('*').order('tier_order')
  ...
  for (const [name, profileId] of Object.entries(PROFILES)) {
    ...
    evolveCheck[name] = { totalGearChecked: allGear.length, violations, sample: allGear.slice(0, 5) }
  }
  ...
  return NextResponse.json({
    migration: { activity_gear_categories_rows: agc?.length, progression_tiers: pt },
