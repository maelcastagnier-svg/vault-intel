// TEMP debug route -- calls extractBestiary() directly against Cucumber's
// real freshly-fetched member object, and writes the result straight to
// player_data via Supabase, bypassing the full /api/player/sync GET handler
// entirely (never triggers runEvolveSkills). Zero Claude cost. Deleted after use.
import { createClient } from '@supabase/supabase-js'
import { NextResponse } from 'next/server'
import { extractBestiary } from '../../player/sync/route'
...
export async function GET() {
  ...
  return NextResponse.json({
    extracted: {
      bestiary_milestone: bestiary.bestiary_milestone,
      bestiary_kills_count: Object.keys(bestiary.bestiary_kills).length,
      bestiary_kills_sample: Object.fromEntries(Object.entries(bestiary.bestiary_kills).slice(0, 10)),
    },
