// TEMP debug route -- calls extractMinions() directly against Cucumber's
...
import { extractMinions } from '../../player/sync/route'
...
export async function GET() {
  ...
  const { error: updateError } = await supabase
    .from('player_data')
    .update({ crafted_generators: minions.crafted_generators, updated_at: new Date().toISOString() })
