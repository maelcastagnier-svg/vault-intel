// TEMP debug route -- calls extractJacobsContests() directly against Cucumber's real
...
import { extractJacobsContests } from '../../player/sync/route'
...
export async function GET() {
  ...
  const jacob = extractJacobsContests(member)

  const { error: updateError } = await supabase
    .from('player_data')
    .update({
      jacob_medals: jacob.jacob_medals,
      jacob_perks: jacob.jacob_perks,
      jacob_unique_brackets: jacob.jacob_unique_brackets,
      jacob_personal_bests: jacob.jacob_personal_bests,
      jacob_contests: jacob.jacob_contests,
  ...
  return NextResponse.json({
    extracted: {
      ...
      jacob_unique_brackets: jacob.jacob_unique_brackets,
      jacob_personal_bests: jacob.jacob_personal_bests,
      jacob_contests_count: Object.keys(jacob.jacob_contests).length,
    },
