// TEMP debug route -- calls extractFishingStats() directly against Cucumber's real
...
import { extractFishingStats } from '../../player/sync/route'
...
export async function GET() {
  ...
  const { error: updateError } = await supabase
    .from('player_data')
    .update({ fishing_stats: fishingStats, updated_at: new Date().toISOString() })
    .eq('profile_id', CUCUMBER_PROFILE_ID)

  return NextResponse.json({
    extracted: fishingStats,
