// TEMP debug route -- dumps member.slayer.slayer_bosses.<boss> directly on Cucumber's
// real profile to verify claimed_levels + per-tier kills structure (path confirmed via
// hypixel-api-reborn reference audit, verified here against real data before coding).
...
export async function GET() {
  ...
  return NextResponse.json({
    slayerTopLevelKeys: Object.keys(member.slayer || {}),
    bossesFound: Object.keys(slayerBosses),
    zombieFull: slayerBosses.zombie,
    spiderFull: slayerBosses.spider,
    wolfFull: slayerBosses.wolf,
    endermanFull: slayerBosses.enderman,
  })
}
