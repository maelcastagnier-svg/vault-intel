...
// residu apres nettoyage d'un bug reel : Haiku confondait parfois l'echelle Pluton
// (1-7) avec une echelle interne du jeu presente dans le contenu source (niveau HOTM,
// SkyBlock Level, niveau Carpentry...), retournant des tier_min/tier_max hors 1-7
// (ex: 500, 200, 0). 2670 lignes corrompues supprimees avant ce redeploiement.
// Fix double : prompt renforce (interdiction explicite de copier un nombre brut de la
// page) + clamp cote code en filet de securite (jamais confier une contrainte
// numerique a la seule sortie structuree du modele).
...
const SYSTEM_PROMPT = `Tu classes des TABLES de donnees de reference du jeu Hypixel Skyblock. Pour chaque table (nom + colonnes + echantillon de lignes), reponds a DEUX questions :
...
2. is_gated -- CETTE TABLE represente-t-elle du contenu DEBLOQUABLE (un vrai palier de progression existe : networth necessaire, niveau XP requis, prerequis de zone/quete documente) ? Une regle UNIVERSELLE, vraie pour tout joueur des le debut independamment de sa progression n'est PAS gated -- reponds is_gated=false.
   Si is_gated=true : donne gate_type et tier_min/tier_max.

   ⚠️ REGLE ABSOLUE sur tier_min/tier_max : c'est TOUJOURS un entier entre 1 et 7 inclus, JAMAIS autre chose.
   Cette echelle 1-7 est la NOTRE (Starter a Master), PAS une echelle presente dans le contenu source.
   Si la table mentionne un niveau HOTM (peut aller jusqu'a 10+), un SkyBlock Level (jusqu'a 500+), un niveau
   de skill/Carpentry (jusqu'a 60+, 200...), ou tout autre numero du jeu -- NE COPIE JAMAIS ce nombre tel quel.
   Convertis-le toi-meme en position dans l'echelle 1-7 (ex: HOTM tier 8/10 -> position ~6/7 -> tier_min=6).
...
// Filet de securite : jamais confier une contrainte numerique a la seule sortie
// structuree du modele (deja vu : minimum/maximum non supportes par le schema
// Anthropic). Clamp dur cote code, independant de ce que le prompt obtient ou non.
    ...
    for (let i = 0; i < tablesInfo.length; i += batchSize) {
      ...
      try {
        ...
        for (const c of classifications) {
          ...
          for (let ri = 0; ri < t.rows.length; ri++) {
            ...
            batchInserts.push({
              ...
              raw_data: row,
              classification_method: 'haiku_table_level_v3',
        ...
        for (let bi = 0; bi < batchInserts.length; bi += 500) {
          ...
          if (error) errors.push({ table: 'batch-insert', error: error.message })
