// TEMP debug route -- calls extractSlayerDetail() directly against Cucumber's real
...
import { extractSlayerDetail } from '../../player/sync/route'
...
export async function GET() {
  ...
  const { error: updateError } = await supabase
    .from('player_data')
    .update({ slayer_detail: slayerDetail, updated_at: new Date().toISOString() })
    .eq('profile_id', CUCUMBER_PROFILE_ID)

  return NextResponse.json({
    extracted: slayerDetail,
