// TEMP debug route -- calls extractChocolateFactory() directly against Cucumber's real
...
import { extractChocolateFactory } from '../../player/sync/route'
...
export async function GET() {
  ...
  const { error: updateError } = await supabase
    .from('player_data')
    .update({ chocolate_factory: chocolateFactory, updated_at: new Date().toISOString() })
    .eq('profile_id', CUCUMBER_PROFILE_ID)

  return NextResponse.json({
    extracted: chocolateFactory,
